...
| `backup-orangepi.bat` | Copia Paperless, ChromaDB, fotos, vídeos, documentos a indexar, la base de datos de Immich y el volumen de Open WebUI a un NAS/Orange Pi por red (`scp`/`rsync`/`pg_dump`/`docker run` vía WSL). Sin tarea programada propia; ver «Qué no resuelve este montaje» en la sección 9 para el detalle y las limitaciones. |
