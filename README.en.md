...
*(Placeholders: screenshots pending in `docs/img/`. Until then these links show no image.)*
...
Copy `scripts/secrets.local.bat.example` to `scripts/secrets.local.bat` and fill in your real Paperless token (Settings → API Tokens in the Paperless-ngx interface) and your Immich key (Settings → API Keys) — the latter is only needed for `listar_personas` in `mcp_fotos.py`, read-only permissions are enough (`person.read`, `asset.read`, `face.read`).
